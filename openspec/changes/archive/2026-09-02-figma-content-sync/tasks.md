...
- [x] 1.1 Не рендерити сірий заголовок секції в хедері
  Files: src/components/site/SiteHeader.vue
  Do: PO override 2026-09-02: не показуй `.site-header__section` на жодному path. Хедер = лого + нав + charity + lang. Не імпортуй `useRoute` для тайтлів. Не міняй `@media (max-width: 1279px)` хедера і не повертай `1727px`.
  Done-when: У `SiteHeader.vue` немає `.site-header__section` і мапи path→тайтл; у файлі немає `1727px`.

## 2. Події

- [x] 2.1 Замінити заголовок сторінки подій
...
- [x] 2.2 Не чіпати embed і лінк на EventsPage
...
- [x] 3.1 Один пробіл у абзаці допомоги армії
...
- [x] 3.2 Замінити байти `army-thumb.png`
...
- [x] 3.3 Не додавати картку Фішки в перелік
...
- [x] 4.2 Назад на Golden Sprints, CTA лишити
  Files: src/pages/GoldenSprintsPage.vue
  Do: Імпортуй `SiteBackLink` і постав `<SiteBackLink tone="violet" />` зліва над фото (Figma `299:64` / PO), не в колонці h1. Існуючий `RouterLink.gs__more` на `/projects/golden-sprints/app` з `readMoreLabel` не видаляй. Не міняй `@media (max-width: 767px)` і копі «Жіночий велоклуб».
  Done-when: У шаблоні є `SiteBackLink` з `tone="violet"` і `to="/projects/golden-sprints/app"`; немає `1727px`.

- [x] 4.3 Новий заголовок army в контенті
...
- [x] 4.4 Army: title iframe, назад, той самий sheet src
...
- [x] 5.4 Зареєструвати hash-маршрут
...
- [x] 6.1 Копі майстерні під Figma 71:12
...
- [x] 6.2 Рядок іконка+текст замість URL-лейбла
...
- [x] 7.1 Замінити байти десяти PNG
...
- [x] 8.1 Не відкочувати layout CSS і non-goals
